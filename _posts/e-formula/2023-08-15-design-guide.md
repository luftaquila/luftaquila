...
질문 연락 중에 제일 많은 유형이기도 하고,  우리 팀 또한 잘 모르는 상황에서 이러한 부품을 선정하면서 시간을 많이 허비했기에 시스템을 처음 설계할 때 어떤 의식의 흐름으로 부품의 스펙을 선정해야 하는지를 적어 보고자 한다.
...
<del><span style="font-size: .8rem">그런데 경험상 대부분의 팀들이 비슷비슷한 제품을 사용하고 있는 것 같긴 하다. 그리고 아무튼 차 굴려봤던 누구네가 이거 쓴다더라 하면 따라 사는 경향이 확실히 짙다.</span></del>
...
출전 클래스를 정하면 HV 전압은 자연스럽게 결정된다. B클래스로 출전하는 팀은 굳이 150~200VDC 이상을 사용하지 않으려 할 것이다. 어차피 출력 제한이 걸려 있는데 굳이 배터리 크기 키워가며 전압을 올릴 필요가 없다. A클래스로 출전하는 팀은 기왕 A클래스인거 150~200V는 좀 아쉽고 600V는 무서우니 그 사이 어딘가로 전압을 정하게 될 것이다.
...
그러나 용량을 늘리려면 어마어마한 개수의 셀이 추가로 필요해진다. 배터리의 용량은 병렬 라인이 결정한다. 용량을 조금 더 올려보고자 7p 구성을 8p로 올리는 순간, 직렬 개수인 70개의 셀이 추가로 필요해진다. 가뜩이나 70s7p도 셀이 490개나 되는데, 70s8p가 되면 560개가 필요해지는 것이다. 우리는 그만큼의 돈이 없었고, 그냥 아슬아슬한 용량을 택했다.

아마 다른 팀 중에는 직렬 라인을 조금 줄이고 병렬 수를 올리는 팀이 있는 것으로 알고 있다. 나쁘지 않은 선택인 것 같다. 아니면 직렬 라인 수만 조금 올리는 것도 방법이다. 결국 모터는 전압과 전류의 곱인 전력을 먹는다. 적절한 전압 범위 내에서 배터리팩의 총 전력량(kWh)을 필요한 수준으로 맞추기만 하면 된다.

이러한 방식으로 HV 배터리의 전압과 용량이 정해진다. 그러나 이는 각각 배터리의 직렬, 병렬 셀 개수에 영향을 받고, 총 사용하는 셀의 개수는 그 둘의 곱이다. 더 많은 셀은 더 비싸고 더 무거운 배터리팩을 의미한다. 가난하고 힘이 약한 우리는 적당한 타협점을 찾을 수밖에 없다. 참고로 우리 배터리 팩의 무게는 거의 드라이버의 몸무게와 맞먹는다.
...
다음은 짜잘한 HV 부품들이다. 차량기술규정을 만족하기 위해 모든 E-Formula 차량은 HVD와 AIR라는 부품이 있어야 한다. 그리고 고전력이 지나가는 HV 배선은 편의점에서 파는 소시지보다도 두껍다. 그리고 비싸다.

우리가 사용한 부품들은 이렇다.

|HVD 및 HV 전선|AIR|
|:-:|:-:|
|HVD: [Amphenol HVSL1000082A](https://www.amphenolpcd.com.cn/products-detail/i-709.html) <br> HV 전선: [LS전선 EEXV-SB 50SQ](https://cdn.komachine.com/media/product-catalog/ls-cable-system_48200_owxsdz.pdf)|[한국리레이 XEV9-10012F](http://www.hankukrelay.co.kr/m21_view.php?idx=15&cate=1&type=7)|
|![]({{ page.assets }}/hvd.png) <br> <del>ㄴㅇㄱ</del>|![]({{ page.assets }}/air.png)|
...
AIR는 무식하게 큰 산업용 릴레이이다. 엄지손가락 한 마디만한 5V 릴레이만 보다가 이런 주먹만한거 보면 내가 지금 뭘 하고 있나 다소 황당하다. 2022년에는 TE Connectivity의 LEV200A4ANA를 AIR로 사용했는데, 올해 한국리레이의 XEV9로 교체했다. 둘레는 비슷하지만 XEV9가 훨씬 더 컴팩트한 높이를 가지고 있어 설계하기 좋다. 가격도 훨씬 싸고 국산 제품이라 배송이 금방금방 온다는 엄청난 장점이 있다.
...
1. 기초 회로이론(V = IR, P = VI, 직렬/병렬 회로의 특성(KCL 및 KVL과 전압 분배), 간단한 RLC 회로, Pull-Up/Pull-Down 회로, Push-Pull과 Open Drain/Open Collector 회로)
2. 릴레이, 다이오드, 선형 레귤레이터 사용법(사용법이랄게 별로 없는 소자들이다.)
3. 트랜지스터와 MOSFET의 스위칭 동작 : 정말 많이 사용한다. 확실히 알아두면 크게 도움이 된다.
4. 555 타이머와 LM311 비교기 IC 사용법 : VI, TSAL, BSPD, RTDS 등에 두루 사용된다.

정말 솔직하게 이 정도면 충분하다고 생각한다. 사전 지식 공부하는데 너무 많은 시간을 들이지 말고, 일단 설계부터 들이받고 보는 걸 추천한다. 회로이론 공부할 시간에 규정 한 시간 더 뒤져보는게 훨씬 낫다.
...
회로도를 그리고 처음 실제로 테스트할 때는 브레드보드를 사용하는 것을 추천한다. 부품 재사용과 교체, 배선 변경이 쉽다. 그러나 차에 달 때는 부품이 덜렁거리는 브레드보드를 쓸 수는 없는 노릇 아닌가? 만능기판에 실제 사용할 기판을 만들면 이런 모습이 된다.

<div class='center'><img src='{{ page.assets }}/perfboard_1.jpg' style='width: 70%'></div>

회로의 극히 일부만 구현한 건데도 제작에 12시간이 걸렸다. 그럴 시간에 그냥 PCB 설계하고 배송을 기다리는 편이 200배 낫다. 깔끔하게 만들기도 어렵다. 나중에 배선이 쌓이면 이게 어떤 전선인지 구분도 안 될 뿐더러, 꽂은 부품의 다리에 전선을 붙이는 방식이다 보니 다리 하나에 전선 세 개씩 붙어 있으면 부품 교체도 어렵다.
