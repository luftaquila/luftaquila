...
## 데이터로거
그런데 사실 데이터로거를 실제로 쓸만한 수준으로 직접 개발하려면 손이 많이 간다. MCU가 CAN 버스 모니터링도 해야 하고, 전원이 꺼져도 수집한 데이터를 가지고 있도록 SD카드 등에 적어놓기도 해야 한다.

여기에 기록한 데이터를 분석하는 도구까지 만드려면 한세월이다. 차 만들기도 바쁜데 대부분이 기계과인 자작차 동네에서 별로 안 친한 코딩과 놀 시간이 없을 것이다.

그래서 직접 개발해 우리 팀 차량에 사용했고, 2023년 대학생 자작자동차대회에서 기술아이디어 금상을 받은 데이터로거를 아무 팀이나 가져다 직접 제작해 쓸 수 있도록 수정하여 공개해 두었다.

프로젝트 이름은 **모노리스**이다. 데이터로거 PCB, 부품 목록, 펌웨어, 설정 도구, 데이터 분석 도구까지 모두 개발하여 전부 오픈소스로 공개되어 있다.

CAN 버스 트래픽, 디지털 및 아날로그 입력 신호, 휠 스피드 센서, 3축 가속도, GPS 위치, LV 전원 전압 등을 측정할 수 있고, 무선으로 데이터를 실시간 모니터링할 수 있다.
