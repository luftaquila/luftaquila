...
for i, photo in enumerate(os.scandir(os.path.join(os.getcwd(), 'photos'))):
  ...
  with open(os.path.join(os.getcwd(), 'photos', photo), 'rb') as f:
    ...
    print(i + 1, '/', len(os.listdir(os.path.join(os.getcwd(), 'photos'))), ' file:', filename + '.' + ext)
    ...
    try:
      ...
      if hasattr(img, 'gps_latitude'):
        ...
        md = re.sub('##location##', f'{map} {img.gps_latitude[0] + img.gps_latitude[1] / 60} {img.gps_latitude_ref}, {img.gps_longitude[0] + img.gps_longitude[1] / 60} {img.gps_longitude_ref} Δ{img.gps_altitude}m  \n<iframe src="https://www.google.com/maps/embed/v1/place?key={GoogleMapsEmbedApiToken}&zoom=17&q={img.gps_latitude[0] + img.gps_latitude[1] / 60},{img.gps_longitude[0] + img.gps_longitude[1] / 60}&center={img.gps_latitude[0] + img.gps_latitude[1] / 60},{img.gps_longitude[0] + img.gps_longitude[1] / 60}" frameborder="0" style="width: 400px; height: 300px; margin: -1rem 0 1rem 50px; border: 0;"></iframe>', md)
      else:
